package com.david5.objects
{
	...
	public class AlgebraicSurface2 extends TriangleMesh3D
	{
		...
		*/
		...
		public function AlgebraicSurface2(material:MaterialObject3D, 
			width:Number = 100, height:Number = 100, 
			segmentsW:Number=0, segmentsH:Number=0, curvaType:Number = 1)
		{
		...
		private function buildSuperficie(width:Number, height:Number, curvaType:Number):void			
		{
			...
			for( var ix:int = 0; ix < gridX + 1; ix++ )
			{
				for( var iy:int = 0; iy < gridY1; iy++ )
				{
					...
					if(1)
					{
						y = 200*Math.sin(y*Math.PI/180);
						x = 100*Math.sin( x*Math.PI/180);
					}					
